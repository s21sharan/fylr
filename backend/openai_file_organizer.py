...
def get_file_summary(file_path):
    """Get summary for a single file using OpenAI"""
    logger.debug(f"Getting summary for file: {file_path}")
    try:
        if is_image_file(file_path):
            logger.debug(f"File is an image: {file_path}")
            # For images, we'll use a simple description based on the filename
...
def generate_file_name(summary, max_length=30):
    """Generate a very concise file name based on the file summary using OpenAI"""
    prompt = {
...
def generate_file_structure(file_summaries):
    """Generate a proposed file structure based on file summaries using OpenAI"""
    try:
...
def analyze_and_organize_files(directory):
    ...
    if file_summaries:
        file_structure = generate_file_structure(file_summaries)
        if file_structure:
        ...
        with open(config_path, 'r') as config_file:
            ...
            if action == 'organize':
                ...
                if not directory or not os.path.isdir(directory):
                    ...
                    sys.exit(1)
                result = analyze_and_organize_files(directory)
